...
class Api {
  ...
  Api(this.name, this._spec, {Map<String, String>? typeAliases})
    ...
    _service = Service(_spec, name, this.typeAliases);
    ...
    for (final pathEntry in _spec.paths.entries) {
      for (final methodEntry
        ...
        var httpMethod = sw.HttpMethod.all.firstWhere(
            (m) => m.name == httpMethodName,
            orElse: () => throw 'Unknown $httpMethodName');
        var path = sw.Path.fromJson(methodEntry.value! as Map<String, Object?>);

        var methodName = _normalizeOperationId(path);
        _service.operations.add(
  ...
  DartType typeFromSchema(sw.Schema schema) {
    ...
    var ref = schema.ref;
    if (ref != null) {
      var complexTypeName = ref.replaceAll('#/definitions/', '');
      var complexType = _spec.definitions[complexTypeName]!;
      if (complexType.type == 'array') {
        return ListDartType(this, parseDartType(complexTypeName));
      } else {
        return parseDartType(complexTypeName);
      }
    } else if (type == 'array') {
      ...
      if (type == 'string' &&
          const ['date-time', 'datetime']
              .contains(schema.format?.toLowerCase())) {
...
class Service {
  ...
  Service(this.spec, String serviceName, TypeAliases aliases) {
    _className = aliases[serviceName] ?? serviceName;
  }
  ...
  String toCode() {
    ...
    if (spec.info.description.isNotEmpty) {
      buffer.writeln(documentationComment(spec.info.description, indent: 0));
    }
    buffer.writeln('''
...
class $_className {
  ...
  $_className({String? socketPath}): 
     _client = ApiClient(socketPath: socketPath, basePath: '${spec.basePath}');
  ...
  String toCode() {
    ...
    var response = (path.responses.entries
            ...
            path.responses.entries
                .firstWhereOrNull((s) => s.key.startsWith('1')))!
        .value;
...
class ComplexType extends DartType {
  ...
  ComplexType(Api api, String name, sw.Schema originalDefinition)
      : definition = _findDefinition(originalDefinition),
  ...
  static sw.Schema _findDefinition(sw.Schema definition) {
    var items = definition.items;
    if (definition.type == 'array' && items != null && items.type == 'object') {
      return items;
    }
    return definition;
  }
